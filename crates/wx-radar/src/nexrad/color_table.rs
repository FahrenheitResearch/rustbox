...
impl ColorTable {
    pub fn build_lut(name: &str, entries: Vec<ColorEntry>, min_value: f32, max_value: f32) -> Self {
    ...
    pub fn from_entries_vec(name: &str, mut entries: Vec<ColorEntry>) -> Self {
        entries.sort_by(|a, b| {
            a.value
                .partial_cmp(&b.value)
    ...
    /// Parse GRLevelX / RadarScope .pal format from a string.
...
/// Manages color table selections (in-memory only, no filesystem).
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
