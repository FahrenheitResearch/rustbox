...
use super::level2::{Level2Sweep, RadialData};
use super::products::RadarProduct;
use super::sites::RadarSite;
...
impl SortedRadialCache {
    pub fn build(sweep: &Level2Sweep) -> Self {
...
impl RadarRenderer {
    ...
    pub fn render_sweep_smooth(
        ...
        let max_range_m = max_slant_range_m * cos_elev;
        let range_km = max_range_m / 1000.0;
        let size = image_size as usize;
        let center = size as f64 / 2.0;
        let scale = center / max_range_m;

        // Sort radials by azimuth for structured traversal
        let cache = SortedRadialCache::build(sweep);
    ...
    fn rasterize_tri(
        ...
        let w0_dx = (p1.1 - p2.1) * inv_denom;
        let w0_dy = (p2.0 - p1.0) * inv_denom;
        let w1_dx = (p2.1 - p0.1) * inv_denom;
        let w1_dy = (p0.0 - p2.0) * inv_denom;
